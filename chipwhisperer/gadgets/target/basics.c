...
void convert_AB_u16(uint16_t* y, uint16_t* x, int n){
...
//http://www.goubin.fr/papers/arith-final.pdf
    ...
    // #if defined(POMMES)
    #if 0
    asm_pommes_convert_AB_u16(y, x, pool);
    #else
...
void convert_BA_u16(uint16_t* y, uint16_t* x, int n){
#if defined(PQM4) && defined (ASM)
//http://www.goubin.fr/papers/arith-final.pdf
...
void sec_and_u16(uint16_t* z, const uint16_t* x, const uint16_t* y, int n){
    ...
    #if defined(POMMES)
    asm_pommes_sec_and_u16(z, x, y, &pool);
    #else
...
void sec_add_u16(uint16_t* z, const uint16_t* x, const uint16_t* y, int n){
  ...
  if (n == 2){
    uint16_t arith_x[n], arith_y[n], arith_z[n];
    convert_BA_u16(arith_x, x, n);
    convert_BA_u16(arith_y, y, n);
    arith_z[0] = arith_x[0] + arith_y[0];
    arith_z[1] = arith_x[1] + arith_y[1];

    convert_AB_u16(z, arith_z, n);
...
#if defined(PQM4)
  int n = 2;
  send_unsigned("N_SHARES =", (uint8_t)n);
#else
  int n = 2;
  printf("N_SHARES = %d\n", n);
#endif
